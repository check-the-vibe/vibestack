...
function harness(enabled=true, Socket=class { addEventListener(){} }) {
  ...
  const window={fetch:async (...args)=>{calls.push(args);return {status:200};},WebSocket:Socket,addEventListener:(name,fn)=>{listeners[name]=fn;}};
...
test('bodies, queries and exception messages never enter diagnostics',async()=>{
  ...
  for(const secret of ['content-sentinel','query-sentinel','exception-sentinel']) assert.ok(!serialized.includes(secret));
...
test('socket logging preserves the native instance prototype required by noVNC',()=>{
  class NativeSocket {
    static OPEN=1;
